...
{
	sealed class RedModule : SceneModuleT<RedContext>
	{
		protected override RedContext CreateDefaultContext()
		{
		...
		{
			Debug.Log( $"<color=red>{GetType().Name}: OnSetup\n");
		...
		{
			RequestReloadScene( new RedContext());
		}
		public void OnNext( Selectable selectable)
		{
			...
			InstantiateModalModule( new YellowContext( (result) => 
			{
				if( result != false)
				{
					RequestLoadScene( new GreenContext());
				}
			}));
		}
		[SerializeField]
